...
def generate_project_pdf_optimized():
    # Output PDF file
    output_path = "project_documentation_optimized.pdf"
    ...
    # Process files with stricter size limits
    dirs_to_exclude = [
    ...
    # Process core application files with stricter limits
        ...
        for file in files:
            if any(file.lower().endswith(ext) for ext in extensions_to_include):
                ...
                if total_chars_added > 800000:  # Stop before reaching 10MB
                    story.append(Paragraph('Document size limit reached. Remaining files not included.', 
                                          ParagraphStyle('LimitNote', parent=styles['Normal'], textColor='red')))
                    break
                
                # Add file header
                story.append(Paragraph(f'File: {rel_path}', file_header_style))
                
                # Read and add file content with stricter limit
                try:
                    with open(file_path, 'r', encoding='utf-8', errors='replace') as f:
                        content = f.read(20000)  # Limit to first 20k chars per file
    ...
    # Build document
    doc.build(story)
    print(f"Optimized PDF generated successfully at {output_path}")
...
def generate_directory_tree(startpath):
    ...
    for root, dirs, files in os.walk(startpath):
        ...
        count = 0
        for file in files:
            if count >= 20:  # Limit files listed per directory
...
if __name__ == "__main__":
    generate_project_pdf_optimized()